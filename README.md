...
数据库查询成功后还会推送 `dataset_ready` 事件，其中包含 `columns`、`rows` 和自动推荐的 `chart` 配置。前端会使用这些结构化数据生成柱状图或折线图，并同时显示结果表格；最终文字回答仍通过 `task_result` 事件单独返回。

后端事件日志默认写入 `logs/agent_trace.log`，按 5 MB 自动轮转并保留 3 个备份。日志记录任务、工具、数据集和错误事件的摘要信息，不记录 SQL 原文、查询结果原文或密钥。
