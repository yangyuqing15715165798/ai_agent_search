...
function App() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        setEvents((current) => [{ ...payload, localTime: new Date().toLocaleTimeString() }, ...current].slice(0, 8));
        if (payload.event === 'task_result') setTaskState('done');
        if (payload.event === 'error') setTaskState('error');
  ...
  async function submitTask(event, selectedModel = model) {
    ...
    try {
      ...
      setTaskState('error');
      setEvents([{ event: 'error', message: error.message, localTime: new Date().toLocaleTimeString() }]);
    }
  }
